...
const title = "Rate Card";
class RateCardList extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  componentDidUpdate() {
    if (this.props.confirm.confirm) {
      if (this.props.confirm.data.length) {
        ...
        this.props.deleteAllCrud("form", "rate-card/delete_all", {
  ...
  deleteData = (id) => {
    this.props.deleteCrud("form", "consultants/rate-card", id);
  };
  ...
  editCrud(data) {
    this.props.history.push(`${this.getBasePath()}/rate-card-form/${data.id}`);
  }
  // changePassword = (data) => {
  ...
  // };
  addCrud() {
    this.props.history.push(`${this.getBasePath()}/rate-card-form/new`);
  }
  // export = () => {
  ...
  render() {
    const columns = [
      ...
      // ),
      TableAction(
        PermissionHelper.checkPermission("delete_rate_card")
          ? this.deleteCrud
          : null,
