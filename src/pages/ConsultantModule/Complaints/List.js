...
class ComplaintsList extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  componentDidUpdate() {
    if (this.props.confirm.confirm) {
      if (this.props.confirm.data.length) {
        ...
        this.props.deleteAllCrud("form", "complaints/delete_all", {
  ...
  deleteData = (id) => {
    this.props.deleteCrud("form", "complaints", id);
  };
  ...
  editCrud(data) {
    this.props.history.push(`/admin/complaints-form/${data.id}`);
  }
  // changePassword = (data) => {
  ...
  // };
  addCrud() {
    this.props.history.push(`/admin/complaints-form/new`);
  }
  // export = () => {
      ...
      <GridContainer>
        <GridItem xs={12}>
          <MaterialDataTable
            ...
            columns={columns}
            deleteAll={
              PermissionHelper.checkPermission("delete_complaints")
                ? this.deleteAll
                : false
            }
            url={`complaints${UserHelper.isConsultant() || id == undefined
              ? ""
