...
const title = "Google Trends List";
class GoogleTrendList extends React.PureComponent {
  constructor(props) {
    super(props);
    ...
    if (this.props.confirm.confirm) {
      if (this.props.confirm.data.length) {
        ...
        this.props.deleteAllCrud(
          "form",
          `google-trends/${this.props.match.params}/delete_all`,
          {
  ...
  deleteData = (id) => {
    this.props.deleteCrud(
      "form",
      `google-trends/${this.props.match.params}`,
      id
    );
  };
  ...
  editCrud(data) {
    this.props.history.push(
      `${this.getBasePath()}/google-trend-form/${data.id}`
    );
  }
  ...
  addCrud() {
    this.props.history.push(`${this.getBasePath()}/google-trend-form/new`);
  }

  pullData() {
    this.props.pullData(
      "form",
      `google-trends/${this.props.match.params.id}/fetch`
    );
  }
  ...
  render() {
    const columns = [
      ...
      TableAction(
        PermissionHelper.checkPermission("delete_vendor_google_trend")
          ? this.deleteCrud
          : null,
      ...
      <GridContainer>
        <GridItem xs={12}>
          <MaterialDataTable
            ...
            pullData={
              PermissionHelper.checkPermission("fetch_vendor_google_trend")
                ? this.pullData
                : null
            }
            url={`google-trends/${this.props.match.params.id}`}
            selection={true}
            grouping={false}
