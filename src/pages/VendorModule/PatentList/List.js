...
const title = "Patent";
class PatentList extends React.PureComponent {
  constructor(props) {
    super(props);
    ...
    if (this.props.confirm.confirm) {
      if (this.props.confirm.data.length) {
        ...
        this.props.deleteAllCrud(
          "form",
          `patent-list/${this.props.match.params}/delete_all`,
          {
  ...
  deleteData = (id) => {
    this.props.deleteCrud("form", `patent-list/${this.props.match.params}`, id);
  };
  ...
  editCrud(data) {
    this.props.history.push(`${this.getBasePath()}/patents-form/${data.id}`);
  }
  ...
  addCrud() {
    this.props.history.push(`${this.getBasePath()}/patents-form/new`);
  }
  ...
  render() {
    const columns = [
      ...
      TableAction(
        PermissionHelper.checkPermission("delete_vendor_patent_list")
          ? this.deleteCrud
          : null,
      ...
      <GridContainer>
        <GridItem xs={12}>
          <MaterialDataTable
            ...
            url={`patent-list/${this.props.match.params.id}`}
            selection={true}
            grouping={false}
