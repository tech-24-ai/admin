...
// material ui icons
import MailOutline from "@material-ui/icons/Pages";
...
import { TYPE } from "_constants/form.constants";
...
class CountryForm extends React.PureComponent {
  ...
  getFormFields = () => {
    ...
    const formFields = [
      ...
      },
      {
        ...
        value: form.type || "",
        icon: "assignment",
        error: this.validator.message("type", form.type, "required|min:1"),
      },
      {
        name: "html",
        label: "Html",
  ...
  handleSubmit(e) {
    ...
    if (this.validator.allValid()) {
      ...
      const { id } = this.props.match.params;
      if (id && id === "new") {
        this.props.create("formData", "pages", data);
      } else {
        this.props.update("formData", "pages", id, data);
      }
      this.resetForm();
      this.goBack();
    } else {
  ...
  bindData = () => {
    const { id } = this.props.match.params;
    if (id && id !== "new") {
      crudService._get("pages", id).then((response) => {
  ...
  render() {
    ...
    const { id } = this.props.match.params;
    let title = "Add New Content Page";
    let btnText = "Create";
    if (id && id !== "new") {
