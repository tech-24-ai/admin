...
class StepForm extends React.PureComponent {
    ...
    handleSubmit(e) {
        ...
        if (this.validator.allValid()) {
            ...
            if (id && id === 'new') {
                this.props.create('formData', 'steps', data)
            } else {
                this.props.update('formData', 'steps', id, data)
            }
            this.resetForm();
            this.goBack();
        } else {
    ...
    bindData = () => {
        const { id } = this.props.match.params
        if (id && id !== 'new') {
            crudService._get('steps', id).then((response) => {
...
export default withStyles(styles)(connect(mapStateToProps, actionCreators)(StepForm));