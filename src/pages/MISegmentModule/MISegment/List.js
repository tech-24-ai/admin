...
const title = 'MI Segments'
class MISegmentList extends React.PureComponent {
    constructor(props) {
        super(props)
    ...
    componentDidUpdate() {
        if (this.props.confirm.confirm) {
            if (this.props.confirm.data.length) {
                ...
                this.props.deleteAllCrud('form', 'mi-segment/delete_all', { ids: JSON.stringify(ids) });
            } else {
    ...
    deleteData = (id) => {
        this.props.deleteCrud('form', 'mi-segment', id);
    }
    ...
    editCrud(data) {
        this.props.history.push(`/admin/mi-segment-form/${data.id}`)
    }
    ...
    addCrud() {
        this.props.history.push(`/admin/mi-segment-form/new`)
    }
            ...
            <GridContainer>
                <GridItem xs={12}>
                    <MaterialDataTable
                        ...
                        search={true}
                        sorting={true}
                        export={PermissionHelper.checkPermission('export_mi_segment') ? this.export : false}
                        filtering={true}
                    />
                </GridItem>
            </GridContainer>
...
export default connect(mapStateToProps, actionCreators)(MISegmentList);