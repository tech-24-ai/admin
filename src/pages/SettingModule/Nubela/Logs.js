...
class NubelaLogsList extends React.PureComponent {
  ...
  editCrud(data) {
    this.props.history.push(`/visitors/nubela-logs/${data.id}`);
  }
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (item) =>
          item.logs && (
            <div>
              {item.logs?.substring(1, 100)}{" "}
              <span>
                <Link
                  onClick={() => this.toggleModal(item.logs)}
                  href="javascript:"
                >
                  ... More
                </Link>
